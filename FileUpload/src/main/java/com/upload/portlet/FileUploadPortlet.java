...
import com.upload.constants.FileUploadPortletKeys;
...
@Component(
	...
	property = {
		...
		"javax.portlet.display-name=FileUpload Portlet",
...
public class FileUploadPortlet extends MVCPortlet {
	...
	public Folder createFolder(ActionRequest actionRequest,ThemeDisplay themeDisplay)
	{
		...
		if (!folderExist) {
			...
			try {
				...
				folder = DLAppServiceUtil.addFolder(repositoryId,PARENT_FOLDER_ID, themeDisplay.getUser().getFullName(),ROOT_FOLDER_DESCRIPTION, serviceContext);
	...
	public boolean isFolderExist(ThemeDisplay themeDisplay){
			...
			try {
				DLAppServiceUtil.getFolder(themeDisplay.getScopeGroupId(), PARENT_FOLDER_ID, themeDisplay.getUser().getFullName());
				folderExist = true;
	...
	public Folder getFolder(ThemeDisplay themeDisplay){
		Folder folder = null;
		try {
			folder =DLAppServiceUtil.getFolder(themeDisplay.getScopeGroupId(), PARENT_FOLDER_ID, themeDisplay.getUser().getFullName());
		} catch (Exception e) {	
			System.out.println(e.getMessage());
		}
		return folder;
...
//          for(uploadPortletRequest.getPortletRequest())
